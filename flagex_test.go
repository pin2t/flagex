...
// --- FlagSet tests ---
// These test the FlagSet(fs) wrapper API, using a standalone flag.FlagSet
// instead of the global flag.CommandLine.

func newTestFlagSet() (*flagSet, *string, *string, *string, *int, *bool, *float64) {
    var fs = flag.NewFlagSet("test", flag.ContinueOnError)
    var user = fs.String("user", "", "user name")
    var url = fs.String("url", "http://localhost", "URL")
    var listen = fs.String("listen", ":8080", "listen address")
    var count = fs.Int("count", 0, "count value")
    var verbose = fs.Bool("verbose", false, "verbose mode")
    var ratio = fs.Float64("ratio", 1.0, "ratio value")
    return FlagSet(fs), user, url, listen, count, verbose, ratio
}

func TestFlagSetParseFile(t *testing.T) {
    var fs, user, url, listen, count, verbose, ratio = newTestFlagSet()
    var path = filepath.Join(t.TempDir(), "test.conf")
    var err = os.WriteFile(path, []byte("# comment\n\nuser=configuser\nurl = http://localhost:8332\nlisten=:9999\ncount=42\nverbose=true\nratio=0.75\n"), 0600)
    if err != nil {
        t.Fatal(err)
    }
    fs.fs.Set("listen", ":7777")
    if err := fs.ParseFile(path); err != nil {
        t.Fatalf("ParseFile: %v", err)
    }
    if *user != "configuser" {
        t.Fatalf("expected user from config, got %q", *user)
    }
    if *url != "http://localhost:8332" {
        t.Fatalf("expected url from config with spaces trimmed, got %q", *url)
    }
    if *listen != ":7777" {
        t.Fatalf("expected previously-set listen to win over config, got %q", *listen)
    }
    if *count != 42 {
        t.Fatalf("expected count=42 from config, got %d", *count)
    }
    ...
    if err := fs.ParseFile(filepath.Join(t.TempDir(), "missing.conf")); err == nil {
        t.Fatalf("expected error for missing file")
    }
    os.WriteFile(path, []byte("unknown-option=1\n"), 0600)
    if err := fs.ParseFile(path); err == nil {
...
func TestFlagSetParseStream(t *testing.T) {
    var fs, user, url, listen, count, verbose, ratio = newTestFlagSet()
    fs.fs.Set("listen", ":7777")
    var r = strings.NewReader("user=streamuser\nurl = http://stream:9000\nlisten=:5555\ncount=10\nverbose=false\nratio=2.5\n")
    if err := fs.ParseStream(r); err != nil {
        t.Fatalf("ParseStream: %v", err)
    }
    if *user != "streamuser" {
        t.Fatalf("expected user from stream, got %q", *user)
    ...
    if *listen != ":7777" {
        t.Fatalf("expected previously-set listen to win over stream, got %q", *listen)
    }
    if *count != 10 {
        t.Fatalf("expected count=10 from stream, got %d", *count)
    }
...
func TestFlagSetParseStreamErrors(t *testing.T) {
    var fs, _, _, _, _, _, _ = newTestFlagSet()
    if err := fs.ParseStream(strings.NewReader("unknown-option=1\n")); err == nil {
        t.Fatalf("expected error for unknown option")
    }
    if err := fs.ParseStream(strings.NewReader("no equals sign\n")); err == nil {
        t.Fatalf("expected error for malformed line")
    }
    var errReader = &errorReader{msg: "test read error"}
    if err := fs.ParseStream(errReader); err == nil {
        t.Fatalf("expected error for reader failure")
    }
}

func TestFlagSetParseURL(t *testing.T) {
    var fs, user, url, listen, count, verbose, ratio = newTestFlagSet()
    fs.fs.Set("listen", ":7777")
    var srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        w.Write([]byte("user=urluser\nurl = http://remote:7000\nlisten=:8888\ncount=7\nverbose=true\nratio=3.14\n"))
    ...
    defer srv.Close()
    if err := fs.ParseURL(srv.URL); err != nil {
        t.Fatalf("ParseURL: %v", err)
    }
    if *user != "urluser" {
        t.Fatalf("expected user from URL, got %q", *user)
    ...
    if *listen != ":7777" {
        t.Fatalf("expected previously-set listen to win over URL, got %q", *listen)
    }
    if *count != 7 {
        t.Fatalf("expected count=7 from URL, got %d", *count)
    }
...
func TestFlagSetParseURLErrors(t *testing.T) {
    var fs, _, _, _, _, _, _ = newTestFlagSet()
    if err := fs.ParseURL("http://127.0.0.1:1/nope"); err == nil {
        t.Fatalf("expected error for unreachable URL")
    }
    var srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        w.WriteHeader(http.StatusNotFound)
...
func TestFlagSetParseEnvWithPrefix(t *testing.T) {
    var fs, user, url, listen, count, verbose, ratio = newTestFlagSet()
    t.Setenv("APP_USER", "envuser")
    t.Setenv("APP_URL", "http://env:9000")
    t.Setenv("APP_LISTEN", ":8888")
    ...
    t.Setenv("APP_VERBOSE", "true")
    t.Setenv("APP_RATIO", "1.5")
    if err := fs.ParseEnv("APP_"); err != nil {
        t.Fatalf("ParseEnv: %v", err)
    }
    if *user != "envuser" {
        t.Fatalf("expected user from env, got %q", *user)
...
func TestFlagSetParseEnvPrecedence(t *testing.T) {
    var fs, _, _, listen, _, _, _ = newTestFlagSet()
    fs.fs.Set("listen", ":7777")
    t.Setenv("APP_LISTEN", ":8888")
    if err := fs.ParseEnv("APP_"); err != nil {
        t.Fatalf("ParseEnv: %v", err)
    ...
    if *listen != ":7777" {
        t.Fatalf("expected previously-set listen to win over env, got %q", *listen)
    }
}

func TestFlagSetParseEnvErrors(t *testing.T) {
    var fs, _, _, _, _, _, _ = newTestFlagSet()
    t.Setenv("APP_FOO", "bar")
    if err := fs.ParseEnv("APP_"); err == nil {
        t.Fatalf("expected error for unknown option with prefix")
...
func TestFlagSetParseEnvInvalidValue(t *testing.T) {
    var fs, _, _, _, _, _, _ = newTestFlagSet()
    t.Setenv("APP_COUNT", "notanint")
    if err := fs.ParseEnv("APP_"); err == nil {
        t.Fatalf("expected error for invalid value")
...
func TestFlagSetParseEnvNoPrefix(t *testing.T) {
    var fs, user, url, _, _, _, _ = newTestFlagSet()
    t.Setenv("USER", "envuser")
    t.Setenv("URL", "http://env:9000")
    t.Setenv("SOME_RANDOM_VAR", "should-be-ignored")
...
func TestFlagSetParseEnvPrefixSkipsUnrelated(t *testing.T) {
    var fs, user, _, _, _, _, _ = newTestFlagSet()
    t.Setenv("OTHER_USER", "other")
    t.Setenv("APP_USER", "envuser")
    if err := fs.ParseEnv("APP_"); err != nil {
        t.Fatalf("ParseEnv: %v", err)
